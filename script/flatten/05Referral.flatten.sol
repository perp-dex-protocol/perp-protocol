...
// src/libraries/ReferralsUtils.sol
...
library ReferralsUtils {
    ...
    function registerPotentialReferrer(address _trader, address _referrer) internal {
        IReferrals.ReferralsStorage storage s = _getStorage();
        IReferrals.ReferrerDetails storage r = s.referrerDetails[_referrer];
    ...
    function distributeReferralReward(
        ...
        IReferrals.ReferrerDetails storage r = s.referrerDetails[referrer];
        ...
        IERC20_1(AddressStoreUtils.getAddresses().gns).mint(address(this), referrerRewardGns);

        IReferrals.AllyDetails storage a = s.allyDetails[r.ally];
    ...
    function claimAllyRewards() internal {
        IReferrals.AllyDetails storage a = _getStorage().allyDetails[msg.sender];
        uint256 rewardsGns = a.pendingRewardsGns;

        if (rewardsGns == 0) revert IReferralsUtils.NoPendingRewards();

        a.pendingRewardsGns = 0;
        IERC20_1(AddressStoreUtils.getAddresses().gns).safeTransfer(msg.sender, rewardsGns);
    ...
    function claimReferrerRewards() internal {
        IReferrals.ReferrerDetails storage r = _getStorage().referrerDetails[msg.sender];
     ...
     * @dev Check IReferralsUtils interface for documentation
     */
    function getAllyDetails(address _ally) internal view returns (IReferrals.AllyDetails memory) {
     ...
     * @dev Check IReferralsUtils interface for documentation
     */
    function getReferrerDetails(address _referrer) internal view returns (IReferrals.ReferrerDetails memory) {
     ...
     */
    function _getStorage() internal pure returns (IReferrals.ReferralsStorage storage s) {
        uint256 storageSlot = _getSlot();
        assembly {
            s.slot := storageSlot
        }
    ...
    function _whitelistAlly(address _ally) internal {
        ...
        IReferrals.AllyDetails storage a = _getStorage().allyDetails[_ally];
    ...
    function _whitelistReferrer(address _referrer, address _ally) internal {
        ...
        if (_ally != address(0)) {
            IReferrals.AllyDetails storage a = s.allyDetails[_ally];
    ...
    function _unwhitelistReferrer(address _referrer) internal {
        IReferrals.ReferrerDetails storage r = _getStorage().referrerDetails[_referrer];
...
// src/core/facets/GNSReferrals.sol
